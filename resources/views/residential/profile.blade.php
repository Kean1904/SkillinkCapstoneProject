    ...
    <div class="page-content">
        ...
        <div class="page-inner" style="max-width: 680px;">
            ...
            @if(isset($errors) && $errors->any())
                <div style="background: rgba(239, 68, 68, 0.25); border: 1px solid #f87171; color: #fca5a5; padding: 12px 20px; border-radius: 8px; margin-bottom: 20px; font-size: 13.5px;">
            ...
            <div class="card" style="background: rgba(30, 58, 138, 0.35); border: 1px solid rgba(255,255,255,0.25); border-radius: 16px; padding: 22px;">
                ...
                <div style="display: flex; flex-direction: column; gap: 14px; font-size: 13.5px;">
                    <div style="display: flex; align-items: center; gap: 12px;">
                        ...
                        <div style="flex: 1;"><span style="opacity: 0.7; font-size: 11px; display: block;">USERNAME</span><strong>@ {{ $user->name }}</strong></div>
                    </div>
                    <div style="display: flex; align-items: center; gap: 12px;">
                        <i class="fa-solid fa-location-dot" style="width: 20px; text-align: center; color: #93c5fd;"></i>
                        <div style="flex: 1;"><span style="opacity: 0.7; font-size: 11px; display: block;">BARANGAY</span><strong>Brgy. {{ $user->barangay ?? 'Magalang' }}</strong></div>
                    </div>
                    <div style="display: flex; align-items: center; gap: 12px;">
                        <i class="fa-solid fa-house" style="width: 20px; text-align: center; color: #93c5fd;"></i>
                        <div style="flex: 1;"><span style="opacity: 0.7; font-size: 11px; display: block;">ADDRESS</span><strong>{{ $user->address ?? 'Not Provided' }}</strong></div>
                    </div>
                    <div style="display: flex; align-items: center; gap: 12px;">
                        <i class="fa-solid fa-phone" style="width: 20px; text-align: center; color: #93c5fd;"></i>
                        <div style="flex: 1;"><span style="opacity: 0.7; font-size: 11px; display: block;">CONTACT</span><strong>{{ $user->contact_number ?? 'Not Provided' }}</strong></div>
                    </div>
                    <div style="display: flex; align-items: center; gap: 12px;">
                        <i class="fa-solid fa-envelope" style="width: 20px; text-align: center; color: #93c5fd;"></i>
                        <div style="flex: 1;"><span style="opacity: 0.7; font-size: 11px; display: block;">EMAIL</span><strong>{{ $user->email ?? 'Not Provided' }}</strong></div>
                    </div>
                    <div style="display: flex; align-items: center; gap: 12px;">
                        <i class="fa-solid fa-cake-candles" style="width: 20px; text-align: center; color: #93c5fd;"></i>
                        <div style="flex: 1;"><span style="opacity: 0.7; font-size: 11px; display: block;">AGE / GENDER</span><strong>{{ $user->age ?? 28 }} yrs old &bull; {{ $user->gender ?? 'Resident' }}</strong></div>
                    </div>
                </div>
            </div>
    ...
    <div id="editProfileModal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.65); z-index: 3000; align-items: center; justify-content: center; padding: 20px;">
        <div style="background: #1e293b; border: 1px solid rgba(255,255,255,0.25); border-radius: 18px; padding: 25px; width: 100%; max-width: 520px; color: white; max-height: 90vh; overflow-y: auto;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
                <h3 style="font-size: 18px; font-weight: bold;"><i class="fa-solid fa-user-pen" style="color: #60a5fa;"></i> Edit Profile</h3>
            ...
            <form method="POST" action="{{ route('residential.profile.update') }}" enctype="multipart/form-data">
                ...
                <div style="margin-bottom: 12px;">
                    ...
                    <input type="text" name="address" value="{{ $user->address }}" placeholder="e.g. Purok 3, Rizal St." style="width: 100%; padding: 9px; border-radius: 6px; background: rgba(255,255,255,0.1); color: white; border: 1px solid rgba(255,255,255,0.3);">
                </div>

                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 18px;">
                    <div>
                        <label style="display: block; font-size: 11px; font-weight: bold; margin-bottom: 4px; opacity: 0.85;">AGE</label>
                        <input type="number" name="age" value="{{ $user->age ?? 28 }}" min="15" max="120" style="width: 100%; padding: 9px; border-radius: 6px; background: rgba(255,255,255,0.1); color: white; border: 1px solid rgba(255,255,255,0.3);">
                    </div>
                    <div>
