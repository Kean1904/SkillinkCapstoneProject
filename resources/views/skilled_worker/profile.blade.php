...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: Arial, sans-serif; }
        ...
        .overlay { position: absolute; top: 0; left: 0; width: 100%; min-height: 100vh; background: rgba(20, 55, 130, 0.65); z-index: 0; pointer-events: none; }
    ...
    <div class="page-content">
        ...
        <div class="page-inner">
            ...
            <div class="page-title">
                <span><i class="fa-solid fa-user"></i> WORKER PROFILE DETAILS</span>
            ...
            <div class="card" style="display: flex; gap: 25px; align-items: center; flex-wrap: wrap;">
                <img src="{{ asset('image/MP_Profile.png') }}" alt="Profile" style="width: 100px; height: 100px; border-radius: 50%; background: white; padding: 6px; border: 3px solid #60a5fa;">
                <div>
                    <h2 style="font-size: 22px; margin-bottom: 4px;">{{ $worker->full_name }}</h2>
                    <p style="font-size: 13px; color: #93c5fd; margin-bottom: 8px;">@ {{ $worker->name }} &bull; Accredited Skilled Worker</p>
                    <div>
                        <span style="background: #16a34a; color: white; padding: 3px 10px; border-radius: 12px; font-size: 12px; font-weight: bold;">
                            <i class="fa-solid fa-certificate"></i> {{ $worker->is_verified ? 'PESO Accredited' : 'Pending Verification' }}
                        </span>
                        <span style="background: #eab308; color: black; padding: 3px 10px; border-radius: 12px; font-size: 12px; font-weight: bold; margin-left: 8px;">
            ...
            <!-- DETAILS GRID -->
            <div class="card">
                <h3><i class="fa-solid fa-address-book"></i> Personal & Accreditation Info</h3>
                <hr>
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 18px; font-size: 14px;">
                    <div><span style="opacity: 0.7; font-size: 12px; display: block;">EMAIL ADDRESS</span><strong>{{ $worker->email }}</strong></div>
                    <div><span style="opacity: 0.7; font-size: 12px; display: block;">CONTACT NUMBER</span><strong>{{ $worker->contact_number }}</strong></div>
                    <div><span style="opacity: 0.7; font-size: 12px; display: block;">BARANGAY</span><strong>{{ $worker->barangay }}</strong></div>
                    <div><span style="opacity: 0.7; font-size: 12px; display: block;">STREET ADDRESS</span><strong>{{ $worker->address }}</strong></div>
                    <div><span style="opacity: 0.7; font-size: 12px; display: block;">AGE / GENDER</span><strong>{{ $worker->age ?? 35 }} y/o ({{ $worker->gender ?? 'Male' }})</strong></div>
                    <div><span style="opacity: 0.7; font-size: 12px; display: block;">TESDA CERTIFICATE</span><strong style="color: #60a5fa;">{{ $worker->certificate_proof ?? 'TESDA NC II' }}</strong></div>
                </div>
            </div>
